\todo[inline]{The primary changes are as follows. 
	\begin{enumerate}
		\item  Both reviewers commented that the equations were pointless, and a better description of some other things would be better. I agree, and I know a lot more about them now. So, after removing those, I have added in the image from the thesis showing the motion that helps convey how these things work. Additionally, I have added in the picture of the cycloid plate after testing to show the wear and address those questions. 
		\item I added in a few words in the abstract "lunar or martian" to set it up more explicitly for space.
		\item I added in a few more words to describe why we didn't test to the extremes on the long runs, specifically that the system was designed for short bursts, so the lower values were used for the long runs to get lifetime. 
		\item I fixed typos and such that were pointed out. 
		\item I removed the verbage for the planetary drive in Fig. 9 because it didn't stengthen the argument (pointed out by both reviewers). I don't have the new image on this computer, so I will do that tomorrow. But pretend that the yellow line isn't there. 
		\item I fixed the references, I noticed these on the Thesis and a reviewer pointed them out that some were missing sources. 
		\item NOTE: Without the comments, the paper is 1 line over 8 pages (the max) so after your review, if that is still the case, I will remove something from somewhere or I will make a figure smaller to get it back to 8. 
	\end{enumerate} 
}
...
\todo[inline]{I rephrased the description of the cycloid, and put in the wiki image to make it clearer. I also added the single revolution graphic to make it clearer}
